...
We then generate two $\U([0,1])$ random variables of size $T=1000$ and call \mintinline{python3}{box_muller} to simulate $x \sim \N(0,I)$ where $x = (x_1, x_2)'$. Then note that since every covariance matrix $\Sigma$ is positive-semidefinite, there exists a Cholesky factorization $\Sigma = PP'$, where $P$ is a lower triangular matrix. So 
\[
...
%\begin{figure}[!h]
%\includegraphics[width=\textwidth]{q2-fig}
%\caption{Simulated series for $y_t$ with $T=1000$}
%\label{q2-fig}
%\end{figure}
...
\]
Note that this is equivalent to computing the sample autocovariance and taking its Fourier transform. This estimation procedure is implemented in the function \mintinline{python3}{sample_sdf}. 
\begin{python3code}
...
\end{python3code}    
The function \mintinline{python3}{unif_window} estimates the spectral density function by appling a uniform window with a truncation lag of 10 to the sample spectral density.  
\begin{python3code}              
...
Consider
\begin{align*}
	x_t &= y_{1t} - y_{1,t-1} \\ %\label{eq:xt} 
	z_t &= \frac{1}{3}(y_{1,t-1} + y_{1t} + y_{1,t+1}). % \label{eq:zt}
\end{align*}
...
\end{python3code}
The following code shows the function calls to \mintinline{python3}{sample_sdf},  \mintinline{python3}{unif_window}, and \mintinline{python3}{ar_sdf} used to generate estimates of the spectral densities of the $x_t$ and $z_t$ processes. 
\begin{python3code}
# Sample SDFs
